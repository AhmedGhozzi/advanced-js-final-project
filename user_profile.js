if(user && user.id == getQueryParam("id")){
    document.getElementById("logged-profile").classList.add("active");
}

function showPostDetails(id){
    location.href = `post_details.html?id=${id}`;
}

function fillProfilePage(){
    const userId = getQueryParam("id");
    const pdp = document.getElementById("profile-picture");
    const email = document.getElementById("user-email");
    const name = document.getElementById("user-username");
    const username = document.getElementById("user-name");
    const postCount = document.getElementById("post-count");
    const commentCount = document.getElementById("comment-count");
    const postowner = document.getElementById("post-owner");
    axios
        .get(`https://tarmeezacademy.com/api/v1/users/${userId}`)
        .then((response) => {
            console.log(response);
            pdp.src = response.data.data.profile_image.length ? response.data.data.profile_image : "profile-pics/default.png";
            email.innerHTML = response.data.data.email ? response.data.data.email : "-";
            name.innerHTML = response.data.data.name;
            username.innerHTML = response.data.data.username;
            postCount.innerHTML = response.data.data.posts_count;
            commentCount.innerHTML = response.data.data.comments_count;
            postowner.innerHTML = response.data.data.username;
        })
}

function getPosts(id = getQueryParam("id")) {
    console.log("hellooo from getUserPosts");
    const posts = document.getElementById("user-posts");
    posts.innerHTML = ""
    axios
      .get(`https://tarmeezacademy.com/api/v1/users/${id}/posts`)
      .then((response) => {
        data = response.data.data;
        const user = JSON.parse(localStorage.getItem("user"));
        for (post of data) {
          tags = post.tags;
          let editDeleteBtns = "";
          if(user && user.id == post.author.id){
            editDeleteBtns = `
              <div class="buttons">
                <button type="button" class="btn btn-secondary" data-post='${JSON.stringify(post)}' onclick='fillPostModal(this)'>Edit</button>
                <button type="button" class="btn btn-danger" onclick='fillDeletePost(${post.id})' data-bs-toggle="modal" data-bs-target="#delete-post-modal">Delete</button>
              </div>
            `;
          }
          let p = `
              <div class="card shadow-sm mb-5" data-id=${post.id}>
                          <div class="card-header d-flex align-items-center justify-content-between">
                              <div class="d-flex align-items-center gap-1" style="cursor:pointer;" onclick="showUserPosts(${post.author.id})">
                                  <div style="width: 30px; height: 30px; overflow: hidden;">
                                      <img src="${post.author.profile_image.length ? post.author.profile_image : "profile-pics/default.png"}" alt="" class="rounded-circle" style="object-fit: cover; width: 100%; height: 100%;"/>
                                  </div>
                                  <b>@${post.author.username}</b>
                              </div>
                              ${editDeleteBtns}
                          </div>
                          <div class="card-body" onclick="showPostDetails(${post.id})">
                              <img src="${post.image}" alt="" class="w-100" />
                              <h6 style="color: #999" class="mt-1">${post.created_at}</h6>
                              <h5>${post.title || ""}</h5>
                              <p class="caption">
                                  ${post.body}
                              </p>
                              <hr>
                              <div class="about d-flex flex-wrap" style="display: inline;">
                                  <div class="comments d-flex align-items-center gap-1">
                                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                          class="bi bi-pen" viewBox="0 0 16 16">
                                          <path
                                              d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001m-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708z" />
                                      </svg>
                                      <span>(${post.comments_count}) comments</span>
                                  </div>
                                  <div class="tag ms-3 d-flex" id="tags-${
                          post.id
                        }" style="display: inline;">
                                      ${tags
                                      .map((t) => {
                                      return `<span
                                          class="me-2 bg-secondary text-white p-2 rounded-pill">${t.name}</span>`;
                                      })
                                      .join("")}
                                  </div>
                              </div>
                          </div>
                      </div>
              `;
                  posts.innerHTML = p + posts.innerHTML;
        //   s++;
        }
  })
  .catch((error) => {
    console.error(error);
  })
  }

getPosts();

function fillPostModal(button = null){
    console.log("hello");
    const post = JSON.parse(button.getAttribute('data-post'));
    console.log(post ? JSON.stringify(post) : "no post provided");
    let postId = document.getElementById("post-id");
    const modalTitle = document.getElementById("modal-title");
    const modalMainButton = document.getElementById("confirm-post");
    const title = document.getElementById("post-title");
    const body = document.getElementById("post-desc");
    modalTitle.innerHTML = post ? "Edit Post" : "Create Post";
    modalMainButton.innerHTML = post ? "Save Changes" : "Create Post";
    title.value = post ? post.title : "";
    body.value = post ? post.body : "";
    postId.value = post ? post.id : "";
    const postModal = new bootstrap.Modal(document.getElementById("post-modal"),{});
    postModal.toggle();
  }
  
  function fillDeletePost(postId){
    document.getElementById("post-id-input").value = postId;
  }

fillProfilePage()

function getQueryParam(param) {
    const urlParams = new URLSearchParams(window.location.search);
    return urlParams.get(param);
}